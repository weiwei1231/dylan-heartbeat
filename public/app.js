...
function fetchModelList(){var url=document.getElementById('cfg-target-url').value.trim(),key=document.getElementById('cfg-target-key').value.trim(),tips=document.getElementById('api-save-tips'),wrap=document.getElementById('model-list-wrap'),sel=document.getElementById('cfg-model-select');if(!url||!key){tips.className='setting-tips error';tips.textContent='请先填写中转站 URL 和 Key';tips.style.display='block';return;}fetch('/admin/models?target_url='+encodeURIComponent(url)+'&target_key='+encodeURIComponent(key),{headers:{'Authorization':getAuthHeader()}}).then(function(r){return r.json().then(function(b){if(!r.ok)throw new Error(b.error||'拉取失败');return b;});}).then(function(data){var items=(data.data||[]).map(function(x){return x&&x.id;}).filter(Boolean);if(!items.length)throw new Error('上游没有返回模型列表');sel.innerHTML='<option value="">选择模型</option>';items.forEach(function(id){var o=document.createElement('option');o.value=id;o.textContent=id;sel.appendChild(o);});wrap.style.display='block';tips.className='setting-tips success';tips.textContent='已拉取 '+items.length+' 个模型';tips.style.display='block';}).catch(function(e){tips.className='setting-tips error';tips.textContent=e.message||'拉取失败';tips.style.display='block';});}
function applySelectedModel(){var s=document.getElementById('cfg-model-select'),i=document.getElementById('cfg-model-name');if(s&&s.value)i.value=s.value;}
function saveApiConfig(){var url=document.getElementById('cfg-target-url').value.trim(),key=document.getElementById('cfg-target-key').value.trim(),model=document.getElementById('cfg-model-name').value.trim(),tips=document.getElementById('api-save-tips');fetch('/admin/save',{method:'POST',headers:{'Content-Type':'application/json','Authorization':getAuthHeader()},body:JSON.stringify({target_url:url,target_key:key,model_name:model})}).then(function(r){return r.json().then(function(b){if(!r.ok)throw new Error(b.error||'保存失败');return b;});}).then(function(){tips.className='setting-tips success';tips.textContent='已保存，下一条消息立即使用新核心';tips.style.display='block';setTimeout(function(){tips.style.display='none';},3500);}).catch(function(e){tips.className='setting-tips error';tips.textContent=e.message||'保存失败';tips.style.display='block';});}
